...
int EqualOrSolrQBuilder::init(DualDictWrapper* dict, const SolrQConfig& solr_q_config) {
    init_base(solr_q_config.name(), solr_q_config.solr_field(), solr_q_config.source_name());
    return 0;
}

int EqualOrSolrQBuilder::make_q(const AnalysisResult& analysis_res,
        int analysis_idx,
        std::string& q) {
    q = "";
    ...
    if (source_name == "question") {
        ...
        if (it != analysis_res.info.end()) {
            ...
            INFO_LOG("search filed[%s] not exist in analysis info_map", source_name.c_str());
            return 0;
        }
        q += get_solr_field();
        q.append(":");
